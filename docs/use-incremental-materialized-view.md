...
# Incremental Materialized View in Cloudberry Database
...
:::tip
Comparison between incremental materialized view and normal view:

- Normal view: Normal views do not store data, but dynamically generate results based on the defined SQL statement each time a query is executed. This means that the data on the view needs to be recalculated each time a query is executed, which can be time-consuming when dealing with large amounts of data.
...
# Query performance comparison with normal view

The following example compares the query performance of a normal view and that of an incremental materialized view when processing large datasets in Cloudberry Database. The following example uses the Query 15 test dataset of TPC-H.

## With normal view

1. Create a normal view `revenue0`.
...
We have conducted a TPC-H test to compare the execution time of inserts and queries with normal view and incremental materialized view, respectively, with different data volumes. The result is as follows.

| Data volume | Normal inserts | Inserts with incremental materialized view | Queries with normal view | Queries with incremental materialized view |
| ----------- | -------------- | ------------------------------------------ | ------------------------ | ------------------------------------------ |
