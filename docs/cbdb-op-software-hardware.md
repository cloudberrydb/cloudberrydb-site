...
| ETCD    | 2 cores | 8 GB | SSD      | 10 Gigabit network card (2 preferred) | 2+       |
| FTS     | 2 cores | 8 GB | SSD      | 10 Gigabit network card (2 preferred) | 1+       |
...
- To prevent a high data disk load from affecting the operating system's normal I/O response, you need to mount the operating system and the data disk on separate disks.
...
For the system disk: To avoid interference with the operating system when the data disk is heavily loaded, it should be based on an independent disk.

For data disk: It is recommended to use LVM to manage data disks. According to test, creating an independent logical volume for each physical volume can achieve the best disk performance. Examples are as follows:
...
The names of the mount points must be consecutive, and the mount points of the data disks should be `/data0`, `/data1`, ..., `/dataN`. The data disk should use the XFS file system format. Examples are as follows:
...
- **Network card configuration**
    ...
    The data exchange network is a network used for transmitting business data, which has high requirements on network performance and throughput. In a production environment, two 10 Gbps NICs are generally required, and they will be used after bonding. The bond 4 parameter recommendations are as follows:
...
- **Connectivity requirements**
    ...
    - Connect the management console and the database host in the data exchange network. If there is a firewall device in the network access between the management console and the database host, ensure that the TCP idle connection can be kept for more than 12 hours.
    ...
    - Connect the database client and the application programs that access the database with the database master node in the data exchange network.
...
- **Switch**
    ...
    - A switch convergence ratio of 4:1 is recommended. When the convergence ratio reaches 6:1, most links will be saturated. Significant packet loss starts to occur when the convergence ratio reaches 8:1.
...
- **Hostname configuration**
    ...
    The database and management console do not have mandatory requirements for the host naming rules, but you need to meeting the following conditions:
    ...
    - The hostname is not case sensitive. It is recommended to use all lowercase letters. Using capital letters for the hostname might cause Kerberos authentication to fail.
    - The hostname must be unique among all hosts.

- **Time synchronization configuration**

    Configure the time synchronization service for all hosts, and the time synchronization service should be started when the host starts. Ensure that the time of all hosts is synchronized. Configure the system time zone as the customer's time zone. For Chinese customers, set the time zone to UTC/GMT+08:00.

- **SELinux and firewall configuration**

    Disable the firewall and SELinux. Here is an example:
...
- SSH configuration
    ...
    The recommended configuration for the SSH server side (`/etc/ssh/sshd_config`) is as follows. After you have made the configuration, run `systemctl restart sshd.service` to make it effective.
    ...
    | PermitEmptyPass words  | no   | Empty password is not permitted for login. |
    | UseDNS                 | no   | DNS is not used.     |

### Kernel parameter configuration

Kernel parameters are important factors that affect the stable operation of the database. The management console automatically changes the kernel parameters when initializing the cluster. You need to make sure that no kernel parameters have been manually set after the operating system is deployed. The following is a script that configures kernel parameters:
...
### SSH password-free setting

You need to configure the connection of the root user from the deployment machine to all other servers as password-free. Here is an example:
