...
There is no `LOCK TABLE` in the SQL standard, which instead uses `SET TRANSACTION` to specify concurrency levels on transactions. Cloudberry Database supports that too; see [SET TRANSACTION](/docs/sql-statements/sql-stmt-set-transaction.md) for details.
...
[BEGIN](/docs/sql-statements/sql-stmt-begin.md), [SET TRANSACTION](/docs/sql-statements/sql-stmt-set-transaction.md), [SELECT](/docs/sql-statements/sql-stmt-select.md)
