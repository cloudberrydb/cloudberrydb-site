...
The [REVOKE](/docs/sql-statements/sql-stmt-revoke.md) command is used to revoke access privileges.
...
The `GRANT` command cannot be used to set privileges for the protocols `file`, `gpfdist`, or `gpfdists`. These protocols are implemented internally in Cloudberry Database. Instead, use the [CREATE ROLE](/docs/sql-statements/sql-stmt-create-role.md) or [ALTER ROLE](/docs/sql-statements/sql-stmt-alter-role.md) command to set the `CREATEEXTTABLE` attribute for the role.
...
[ALTER DEFAULT PRIVILEGES](/docs/sql-statements/sql-stmt-alter-default-privileges.md), [REVOKE](/docs/sql-statements/sql-stmt-revoke.md)
