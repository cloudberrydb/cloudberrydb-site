...
> **Note** A view that is simple enough to be automatically updatable (see [CREATE VIEW](/docs/sql-statements/sql-stmt-create-view.md)) does not require a user-created rule in order to be updatable. While you can create an explicit rule anyway, the automatic update transformation will generally outperform an explicit rule.
...
[ALTER RULE](/docs/sql-statements/sql-stmt-alter-rule.md), [DROP RULE](/docs/sql-statements/sql-stmt-drop-rule.md)
