...
You may use this utility if Cloudberry Database is reporting `Out of memory` errors and causing segments to go down or queries to fail. You collect the memory usage information of one or multiple servers within the Cloudberry Database cluster with `gpmemwatcher` and then use [gpmemreport](/i18n/zh/docusaurus-plugin-content-docs/current/db-utilities/db-util-gpmemreport.md) to analyze the files collected.
...
[gpmemreport](/i18n/zh/docusaurus-plugin-content-docs/current/db-utilities/db-util-gpmemreport.md)
