...
`TRUNCATE` quickly removes all rows from a table or set of tables. It has the same effect as an unqualified [DELETE](/i18n/zh/docusaurus-plugin-content-docs/current/sql-stmts/sql-stmt-delete.md) on each table, but since it does not actually scan the tables it is faster. Furthermore, it reclaims disk space immediately, rather than requiring a subsequent [VACUUM](/i18n/zh/docusaurus-plugin-content-docs/current/sql-stmts/sql-stmt-vacuum.md) operation. This is most useful on large tables.
...
[DELETE](/i18n/zh/docusaurus-plugin-content-docs/current/sql-stmts/sql-stmt-delete.md)
